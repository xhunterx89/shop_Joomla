...
//			echo "<br/>".$values[0]." = ".$values[1];
		...
		if($this->_inline_tocoords == 0 && !empty($this->_mp->toaddress))	{
			...
			if ($tocoord=='') {
				...
				list ($this->_mp->tolat, $this->_mp->tolon) = explode(",", $tocoord);
				$this->_inline_tocoords = 1;
			}
		}
		...
		// Add code to text
		$code = "\n<!-- Plugin Google Maps version 2.20 by Mike Reumer ".(($this->debug_text!='')?$this->debug_text."\n":"")."-->".$code;
	...
	function _processMapv2() {
		...
		if ($this->_mp->proxy=="1") {
			if (substr($this->jversion,0,3)=="1.5")
				$code .= "\nvar proxy = '".$this->base."/plugins/system/plugin_googlemap2_kmlprxy.php?';";
			else
				$code .= "\nvar proxy = '".$this->base."/plugins/system/plugin_googlemap2/plugin_googlemap2_kmlprxy.php?';";
		}
